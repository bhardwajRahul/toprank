...
Use this for account access and the local context shared by Google Ads skills.
...
- Use the authenticated workspace and live account information to establish
  ...
  infer access to Google Ads from another connected platform.
...
The Google Ads preference is `accountId`. Preserve other fields when saving an
...
otherwise it is `~/.notfair/`. Google Ads business context and personas live under
`{data_dir}/`. Create directories only when the requested workflow needs to
